<script>
  ...
  function hilightText(event) {
    ...
    const highlightedContent = content.replace(regex, (match, content1, content2, content3, content4) => {
      const formulaContent = content1 || content2 || content3 || content4;
      const result = generate_formula(formulaContent);
